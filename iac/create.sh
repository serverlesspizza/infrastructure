...
function waitForStackCreateComplete() {
	...
	aws cloudformation wait stack-create-complete --stack-name $1 --region $3 --profile $2
...
aws ssm put-parameter --name hosted_zone_id --value Z05239482ED1LABEJRIRN --type String --overwrite --region eu-west-1 --profile aws-serverlesspizza-nonprod
aws ssm put-parameter --name hosted_zone_id --value Z039670837H4PA0U0UPFC --type String --overwrite --region eu-west-1 --profile aws-serverlesspizza-prod

# Create the certificate in us-east-1
aws cloudformation create-stack --stack-name 'serverlesspizza-certificate' \
...
# Create the pipeline
aws cloudformation create-stack --stack-name 'serverlesspizza-infrastructure-pipeline' \
